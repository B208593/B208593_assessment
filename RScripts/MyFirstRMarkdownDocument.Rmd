...
#### Tabulate marker test data for the report
...
### -Save ae_type1_performance marker test data to the working data folder 'Data'
...
We need to select widgets (eg. button, dropdown or textbox) to collect our test data in Python.
To use the widget framework, we need to import the ipywidgets Python package.In order to select the widgets we need to check the data type of the variables.
...
The consent data type has 2 options, TRUE or FALSE (logical data type) so boolean widget (Checkbox widget) will be used that are designed to display a boolean value.
...
## **Save the captured test data to the ‘RawData’ folder as well.**  
...
## **3. View the CollectedData ae_attendances data**
...
* **attendances:** the number of attendances for type 1 department at this organisation for this month.    
*	**breaches:** the number of attendances that breached the four-hour target.
*	**performance:** the performance `([1 - breaches]/attendances)` calculated for type 1 A&E departments.
*	**consent:** the consent from the end-user to process and share the data collected with the data capture tool.

## **4. Build a data dictionary for the data collected by the data capture tool**
...
## **7. Append data dictionary to the CollectedData**
...
We are using the 'incorporate_attr()' function to return an R dataset containing metadata stored in its attributes.
