...
prop<-(1-(15/nrow(ae2)))
#The proportion of the raw that needs to be assigned to the training data to ensure there is only 10 to 15 records in the test data.
print(prop)
```
...
### **Split dataset**
We use the createDataPartition() function from the caret package to split our raw data into test and training data sets. 
...
To use the widget framework, we need to import the ipywidgets Python package.In order to select the widgets we previously checked the data type of the variables.
...
### **Period variable**
Data type is string, character. DatePicker widget was selected to capture/collect period variable.

### **Org_code variable**
Data type  is string, character. Selection widget was selected to capture this variable to display single selection list.
...
## **4. Linker**
We use the `build_linker()` function from the *dataMeta* package to construct an intermediary (linker) data frame between the CollectedData and the data dictionary. 
...
## **5. Data dictionary**
We use the `build_dict()` function from the *dataMeta* to constructs a data dictionary for a CollectedData data frame with the aid of the linker data frame between.
