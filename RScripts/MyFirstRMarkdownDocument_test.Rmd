...
## **Viewing the ae_attendances data**
...
*	**org_code:** the Organisation data service (ODS) code is a unique code for the organisation created by the Organisation data service within [NHS Digital](_https://www.digitalsocialcare.co.uk/latest-guidance/how-to-find-your-ods-code/), and used to identify organisations across health and social care.
...
### Tablulate the raw data for the report
...
## Selecting variables for data capture tool 
The variables I have selected for the data capture tool:Index, period, org_ code, attendance, breaches and performance
...
### Save ae_type1_performance training data to your working data folder 'Data'
```{r write,message = FALSE, warning = FALSE, results='hide'}
write_csv(ae2Train, here("Data", "ae_type1_performance_train_full.csv"))
```

### Extract the ae_type1_performance test data. All records that are not in the trainIndex (`-trainIndex`) are assigned to the test data.
...
#### Tabulate marker test data for your report
...
### Save our ae_attendances_ENG_4hr_perfom marker test data to our working data folder 'Data'
...
### We then need to set aside the remaining records to test the data-capture tool.
...
### Finally save our ae_type1_performance test data to our working data folder 'Data'
...
We need to set up an empty data frame in the working data folder to collect the data captured by the Juypter widgets containing only the variables we want to capture
...
The first variable contains the index number, that allows us to connect the test data to the original data set “../RawData/ae_type1_performance_test_full”.
Indexing in Python is a way to refer the individual items by its position.
...
### **Consent**
Consent is a vital area for data protection compliance.
The consent data type has 2 options, TRUE or FALSE so boolean widget (Checkbox widget) will be used that are designed to display a boolean value.
...
Data type is numeric, integer (real values without decimal points). Numeric widget was selected (IntText) for displaying integers.
...
Saving the data collected by the data-capture tool to the working data folder:../Data/CollectedData.csv

## Save the captured test data to the ‘RawData’ folder.
...
## **Data**
Test data collected by the Jupyter Notebook  \"./ipynbScripts/CollectingDataUsingInteractiveJupyterWidgets.ipynb\".  
